...
use std::ops::{SubAssign, AddAssign, Sub, Add};
...
impl FromStr for Action {
  ...
  fn from_str(s: &str) -> Result<Self, Self::Err> {
    ...
    let arg_num = arg.parse::<i32>().map_err(|_| "Unable to parse arg")?;

    match act {
      'N' => Ok(Action::North(arg_num)),
      'S' => Ok(Action::South(arg_num)),
      'E' => Ok(Action::East(arg_num)),
      'W' => Ok(Action::West(arg_num)),
      'L' => Ok(Action::Left(Angle::from_str(arg)?)),
      'R' => Ok(Action::Right(Angle::from_str(arg)?)),
      'F' => Ok(Action::Forward(arg_num)),
...
fn generator(input: &str) -> Vec<Action> {
  input.lines().map(|line| Action::from_str(line).expect("Unable to parse action")).collect()
}
