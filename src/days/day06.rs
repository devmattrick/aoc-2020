...
fn part1(input: &[String]) -> u32 {
  input.iter().map(|group| {
    group.lines().join("").chars().unique().count() as u32
  }).sum::<u32>()
}
...
fn part2(input: &[String]) -> u32 {
  input.iter().map(|group| {
    ...
    let res = first_person.chars().map(|c| group_string.matches(c).count())
      .filter(|&count| count == members).collect::<Vec<usize>>().len();
