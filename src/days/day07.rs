      ...
      match record.as_rule() {
        Rule::rule => {
          ...
          inner_bags.for_each(|inner_bag| {
            ...
            let count = inner_bag.next().unwrap().as_str().parse::<u8>().expect("Unable to parse string into u8");
            let color = inner_bag.next().unwrap().as_str().to_owned();

            bag.push(BagContents {
              color,
              count,
            });
          });
...
fn part1(input: &Graph<String, u8>) -> usize {
  let target = input.node_indices().filter(|node| input.node_weight(*node).unwrap() == "shiny gold").next().unwrap();
